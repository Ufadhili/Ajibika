...
# copying files from server to s3: s3cmd put --recursive images
# s3://ajibika-test/aji-media/
...
def backup_db():
    ...
    with settings(warn_only=True):
        ...
        if dump.failed:
            ...
            upload = run(
                "s3cmd put --recursive %s s3://ajibika/postgresql-backups/" % (sql_file))
            if upload.failed:
...
def config_aws_envs():
    with settings(warn_only=True):
        ...
        AWS_SECRET_ACCESS_KEY = prompt(
            "Please the enter the AWS_SECRET_ACCESS_KEY")
        local_env = local("export AWS_SECRET_ACCESS_KEY=%s" %
                          (AWS_SECRET_ACCESS_KEY))
        if local_env.failed and not confirm("Local Env export has failed. Continue anyway?"):
            abort("Aborting by user request")
        remote_env = run("export AWS_SECRET_ACCESS_KEY=%s" %
                         (AWS_SECRET_ACCESS_KEY))
        if remote_env.failed and not confirm("Remote Env export has failed. Continue anyway?"):
            abort("Aborting by user request")
...
def install_base_packages():
    ...
    package_install(
        "python-software-properties git-core python-dev python-all-dev python-setuptools libapache2-mod-wsgi")
...
def install_postgres_postgis():
    package_install(
        "postgresql postgresql-client postgresql-contrib pgadmin3 postgis postgresql-9.3-postgis-2.1")
...
def configure_nginx():
    ...
    with settings(warn_only=True):
        if file_exists("/etc/nginx/sites-available/www.ajibika.org"):
            ...
            result = put(
                "conf/www.ajibika.org", "/etc/nginx/sites-available/", use_sudo=True)
    ...
    if not file_exists("/etc/nginx/sites-enabled/www.ajibika.org"):
        with settings(warn_only=True):
            result = run(
                "sudo ln -s /etc/nginx/sites-available/www.ajibika.org /etc/nginx/sites-enabled/www.ajibika.org")
...
def configure_gunicorn():
    ...
    with settings(warn_only=True):
        ...
        if result.failed and not confirm("Unable to copy ufadhili.conf to /etc/init/. Continue anyway?"):
            abort("Aborting at user request.")
        puts(
            green("Now adding a soft link to upstart for the ufadhili.conf job"))
...
def install_local_packages():
    local(
        "sudo apt-get install python-software-properties git-core python-dev python-all-dev python-setuptools libapache2-mod-wsgi")
    local(
        "sudo apt-get install build-essential python-pip nginx libgdal1-dev python-gdal gdal-bin")
...
def hard_reset_remote():
    with cd("%sufadhili/" % (WEB_ROOT)):
        result = run(
            "sudo git fetch --all && sudo git reset --hard origin/develop")
